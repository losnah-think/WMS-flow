...
export const flowData: Record<string, Flow> = {
  ...
  return: {
    ...
    steps: [
      ...
      // 2단계: OMS 반품 접수 및 정책 검증
      { from: 'oms_system', to: 'oms_system', label: '[2] OMS 반품 접수 및 정책 검증', desc: 'OMS에서 반품 정책 자동 검증 및 승인 결정', detail: 'FULGO OMS가 반품 정책 자동 검증: ① 반품 가능 기간 확인(구매 후 30일 이내 등) ② 반품 불가 조건 체크(기한초과/고객 귀책/불가 상품) ③ 승인/거부 결정 ④ 승인 시: 택배사 회수 요청 생성, 반품 송장 발행, 반품 정보 등록 ⑤ 거부 시: 고객 거부 통보. 상태: 승인 또는 거부', actor: 'FULGO OMS', term: '검증 완료', features: ['RET-001', 'STK-008'] },
  ...
  storage: {
    ...
    steps: [
      ...
      // 3단계: 로케이션 정책 설정
      { from: 'warehouse_manager', to: 'fulgo', label: '[3] 로케이션 구조 설정', desc: '존-로케이션 체계 정의 및 FULGO 등록', detail: '창고장이 로케이션 정책을 FULGO에 등록: 존(Zone) - 대분류 구역(의류존/악세서리존/신발존/보류존/불량존), 로케이션 - 세부 위치코드(A-01-01 형식). 각 로케이션의 크기, 적재 중량, 속성 정보 입력. FIFO/FEFO 정책 설정 및 FULGO에 반영.', actor: '창고장', term: '정책 등록', features: ['CFG-001', 'STK-006', 'STK-013'] },
      
      // 4단계: 로케이션 할당 지시
      { from: 'fulgo', to: 'worker', label: '[4] 로케이션 할당 및 적치 지시', desc: '상품 속성 기반 로케이션 자동 할당 및 작업 지시', detail: 'FULGO가 입고 상품에 대해 로케이션 자동 할당 및 작업자에 지시: ① 상품 속성(카테고리/크기/수량) 확인 ② 적합한 존 선택 ③ 빈 로케이션 자동 검색 ④ 로케이션 할당 완료 ⑤ 작업자에게 적치 위치 지시 전달. 존 포화 시 자동 알림 및 재배치 제안.', actor: 'FULGO WMS', term: '지시 발행', features: ['STK-006', 'STK-013'] },
      
      // 5단계: 재고 이동 이력 기록
      { from: 'worker', to: 'fulgo', label: '[5] 재고 적치 및 이동 완료 보고', desc: '작업자의 재고 이동/적치 작업 완료 보고 및 이력 기록', detail: '작업자가 적치/이동 작업 완료 후 FULGO에 보고: ① 작업 완료(바코드 스캔으로 확인) ② FULGO가 모든 재고 이동 자동 기록: 입고(OMS→WMS→로케이션 A)→이동(로케이션 A→B)→피킹(로케이션 A→피킹 존)→출고(피킹 존→출하)→반품(반품 존→로케이션 A) ③ 로트/시리얼 번호 기반 추적 ④ 전체 이력 조회 가능', actor: '작업자', term: '완료 보고', features: ['STK-005', 'STK-013'] },
      
      // 6단계: 재고 정책 관리
      { from: 'warehouse_manager', to: 'fulgo', label: '[6] 재고 운영 정책 설정', desc: '화주별/상품별 재고 운영 정책 관리', detail: '창고장이 FULGO에 재고 운영 정책 설정: ① 화주별 정책-안전 재고(상품별), FIFO/FEFO 회전 정책, 로트/유효기한 관리 여부 ② 재고 분류-ABC 분석(A:고빈도 20%, B:중빈도 30%, C:저빈도 50%) ③ 보유 비용-보관료(일/월 단위), 장기 보관 추가 비용, 존별 단가 설정. FULGO가 정책 반영하여 자동 운영.', actor: '창고장', term: '정책 설정', features: ['CFG-001', 'USER-001'] },
      
      // 7단계: 실사 지시
      { from: 'warehouse_manager', to: 'worker', label: '[7] 실사 지시', desc: '창고장이 작업자에게 재고 실사 지시', detail: '창고장이 주기적(주/월 단위) 또는 필요 시 실사 지시 발행: ① 실사 대상 범위(전체/존별/상품별) 결정 ② 실사 기간 설정 ③ 실사 담당자 배정 ④ 작업자에게 실사 지시 전달(실사표 생성). 상태: 실사 대기', actor: '창고장', term: '지시 발행', features: ['STK-010', 'STK-013'] },
      
      // 8단계: 실사 실행
      { from: 'worker', to: 'fulgo', label: '[8] 재고 실사 실행', desc: '작업자가 현장 재고 실사 수행', detail: '현장 작업자가 실사 수행: ① 실사 대상 구역 이동 ② 상품별 바코드 스캔 및 수량 확인 ③ 손상/파손/변형 여부 확인 ④ 실사 현황 실시간 입력(모바일 디바이스) ⑤ 실사 완료 시 FULGO에 제출. 상태: 실사 완료', actor: '작업자', term: '실사 완료', features: ['STK-010', 'STK-005'] },
      
      // 9단계: 실사 결과 분석 및 보고
      { from: 'fulgo', to: 'shipper', label: '[9] 실사 결과 보고 및 분석', desc: '실사 결과를 WMS 재고와 비교 및 화주사 보고', detail: 'FULGO WMS가 실사 결과 자동 분석: ① 실사 수량 vs WMS 수량 비교 ② 차이 발생 현황(증가/감소/손실) 분석 ③ 이상 항목 리스트 생성(도난 의심/오류/파손) ④ 실사 결과 리포트 생성 ⑤ 화주사에게 실사 결과 보고(차이율 %, 문제 항목, 권고사항). 상태: 검증 대기', actor: 'FULGO WMS', term: '보고 완료', features: ['STK-010', 'RPT-001'] },
      
      // 10단계: 수량 조정
      { from: 'worker', to: 'fulgo', label: '[10] 재고 수량 조정', desc: '작업자가 실사 결과 기반 수량 조정 실행', detail: '현장 작업자가 창고장 승인 하 실사 결과에 따라 수량 조정: ① 증가분(입고 누락 등)→시스템에 입력(입고 처리) ② 감소분(도난/파손 등)→시스템에 입력(폐기/차감 처리) ③ 이동 필요 상품→로케이션 재배치 ④ 조정 완료 후 바코드 스캔으로 확인 ⑤ FULGO에 조정 완료 보고. 상태: 조정 완료', actor: '작업자', term: '조정 완료', features: ['STK-011', 'STK-005'] },
      
      // 11단계: 조정 결과 확정 및 OMS 동기화
      { from: 'fulgo', to: 'oms_system', label: '[11] 수량 조정 확정 및 OMS 동기화', desc: '조정 결과를 최종 확정 및 OMS 동기화', detail: 'FULGO WMS가 수량 조정 최종 처리: ① 조정 내용 검증(증감액 재확인) ② 재고 상태 최종 변경(=실사 수량으로 확정) ③ 조정 로그 기록(일시/담당자/사유/수량 변화) ④ OMS로 최종 동기화 전송(WMS 재고 수량 = OMS 수량) ⑤ 정합성 재검증. 상태: 조정 확정', actor: 'FULGO WMS', term: '동기화 완료', features: ['STK-002', 'STK-011'] },
      
      // 12단계: 재고 가시성 제공
      { from: 'fulgo', to: 'shipper', label: '[12] 실시간 재고 모니터링 대시보드', desc: '화주사에게 실시간 재고 현황 제공', detail: 'FULGO 대시보드에서 화주사에게 실시간 재고 현황 제공: ① 전체 재고 현황(가용/예약/보류/불량 비율) ② 존별 재고 분포 및 가동률 ③ 화주별 재고 현황 및 변동 추이 ④ 재고 회전율 ⑤ 최근 실사 결과(정합성 %). 자동 알림: 재고 부족(안전 재고 이하)→알림, 계절 상품 잔여(시즌 종료 30일 전)→알림, 장기 재고(90일 이상 미출고)→알림', actor: '화주사', term: '모니터링', features: ['RPT-001', 'RPT-005'] },
      
      // 13단계: 재고 KPI 리포팅
      { from: 'fulgo', to: 'shipper', label: '[13] 재고 KPI 리포팅', desc: '일간/주간/월간 KPI 자동 생성 및 제공', detail: 'FULGO 자동 생성 리포트: ① 재고 정확도(%) - 실사 기반 정합성율 ② 재고 회전율 - 입고 대비 출고 비율 ③ 평균 체류 기간 - 입고부터 출고까지 소요 일수 ④ 존별 가동률(%) - 용량 대비 사용률 ⑤ 재고 가치 변동 - 수량×단가 ⑥ 손실액 분석 - 파손/도난/손실 규모. 화주별 리포트, 추세 분석, 개선 권고안 포함.', actor: 'FULGO WMS', term: '리포팅 완료', features: ['RPT-001', 'RPT-005'] },
    ]
  }
};